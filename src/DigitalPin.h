...
class DigitalPin {
 public:
    ...
    inline bool value() const { return digitalRead(num) xor inactiveState; }
    inline bool isOn() const { return value(); }
    inline bool isOff() const { return !value(); }
    inline operator bool() const { return value(); }
    inline void setValue(bool value) { digitalWrite(num, inactiveState xor value); }
    inline void set(bool value) { setValue(value); }
    inline void on() { setValue(true); }
    inline void off() { setValue(false); }
    inline void change() { setValue(!value()); }

    inline void setMode(uint8_t mode) { pinMode(num, mode); }

    using FunctionPtr = void (*)();
    using ObjectPtr = void*;
    using MethodPtr = void (*)(ObjectPtr);

    inline void detachInterrupt() { ::detachInterrupt(digitalPinToInterrupt(num)); }
